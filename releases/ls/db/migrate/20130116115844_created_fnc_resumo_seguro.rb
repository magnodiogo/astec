class CreatedFncResumoSeguro < ActiveRecord::Migration
  def self.up
    execute "
create type tp_resumo_seguros as 
(
  mes varchar,
  status varchar,
  valor float
...
create or replace function fnc_resumo_seguros(tipo varchar) 
...
language plpgsql"
  ...
  def self.down
    execute " DROP FUNCTION fnc_resumo_seguros(character varying);
              DROP TYPE tp_resumo_seguros;"
  end
end
