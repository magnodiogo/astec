class CreateFncResumoSegurosImob < ActiveRecord::Migration
  def self.up
    execute "

CREATE OR REPLACE FUNCTION fnc_resumo_seguros_imob(tipo character varying, imob integer)
...
$BODY$
 ...
 begin
    ...
    where date_part('month', created_at) = vmes and date_part('year', created_at) = vano
       and tipo_de_seguro = tipo and status = 1 and imobiliaria_id = imob;
